...
namespace MoreMountains.Feel
{
    ...
    public class Barbarian : MonoBehaviour
    {
        ...
        {
            if (Input.GetKeyDown(ActionKey) || Input.GetKeyDown(ActionKeyAlt) || Input.GetMouseButtonDown(0) || Input.touchCount > 0) {
                if(!allDie) Attack();
            }
        }
        ...
        protected virtual void AcquireTargets()
        {
            ...
            var gameObjects = FindObjectsOfType<BarbarianEnemy>();
            
            foreach (var hitCollider in gameObjects)
            {
